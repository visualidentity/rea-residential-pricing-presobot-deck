function performanceOverTimeRenderCallback($pageContainer) {
	...
	this.renderData = function(data, dropdownSelection, channel) {
		if(data.items.length > 0) {
            ...
            if(channel === 'Buy') {
				$($elems[5]).find('.comparison__title').html('Av. Sold Price');
				$($stats[4]).find('.stat-new-title').html('Total Listings Sold:');
            }
            else {
				$($elems[5]).find('.comparison__title').html('Av. Weekly Rent');
				$($stats[4]).find('.stat-new-title').html('Total Listings Leased:');
            }
